...
public class Perso extends JDialog implements ActionListener {
  ...
  public Perso(Frame frame, String title, boolean modal, int hauteur, int largeur, int mines) {
    super(frame, title, modal);
    H=hauteur;
    L=largeur;
    M=mines;
    demin=(DeminorGame) frame;
    try {
      jbInit();
      pack();//ajuste la dimension
    }
    catch(Exception ex) {
      ex.printStackTrace();
    }
  ...
  public void actionPerformed(ActionEvent e) {
    if (e.getSource()==ok) {
      try {
        ...
        System.gc();
        DeminorGame demineur = new DeminorGame(nH,nL,nMines,4);
      }
